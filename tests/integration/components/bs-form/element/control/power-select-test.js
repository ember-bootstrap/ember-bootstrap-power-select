...
module(
  ...
  function (hooks) {
    ...
    hooks.beforeEach(function () {
      ...
      this.set('PowerSelectControl', PowerSelectControl);
    });

    test('it renders as blockless element', async function (assert) {
      await render(hbs`
    <BsForm @model={{this.model}} as |form|>
      <form.element @controlType="power-select" @controlComponent={{this.PowerSelectControl}} @property="prop" @options={{this.options}} />
    </BsForm>`);
    ...
    test('it renders as blockless control component', async function (assert) {
      await render(hbs`
    <BsForm @model={{this.model}} as |form|>
      <form.element @controlType="power-select" @controlComponent={{this.PowerSelectControl}} @property="prop" @options={{this.options}} as |el|>
        {{el.control}}
      </form.element>
    </BsForm>`);
      assert.dom('.ember-power-select-trigger').exists({ count: 1 });
    ...
    test('it can render array of objects with objectLabelPath', async function (assert) {
      ...
      await render(hbs`
    <BsForm @model={{this.model}} as |form|>
      <form.element @controlType="power-select" @controlComponent={{this.PowerSelectControl}} @property="prop" @options={{this.options}} @optionLabelPath="title" />
    </BsForm>`);
      assert.dom('.ember-power-select-trigger').exists({ count: 1 });
      assert.dom('.ember-power-select-selected-item').hasText('foo');
      await clickTrigger();
    ...
    test('it passes power-select options', async function (assert) {
      ...
      await render(hbs`
    <BsForm as |form|>
      <form.element @controlType="power-select" @controlComponent={{this.PowerSelectControl}} @property="prop2" @options={{this.options}} @placeholder="something" as |el|>
    ...
    test('it passes HTML attributes', async function (assert) {
      ...
      await render(hbs`
    <BsForm as |form|>
      <form.element @controlType="power-select" @controlComponent={{this.PowerSelectControl}} @property="prop" as |el|>
